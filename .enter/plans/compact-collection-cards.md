...
- [ ] 实现首页 9 条预览及展开后/合集 24 条分页，跨页选择与全量搜索不丢失。
- [ ] 搜索文本索引和分类计数复用，减少无关卡片重复渲染。
- [ ] 建立桌面 3 个/手机 1 个卡片 iframe 并发上限，离屏、详情开启和卸载正确释放。
- [ ] 移除固定 2.4 秒 iframe 显示延迟，保留慢加载/失败反馈。
- [ ] 视频和轮播只为可见内容保留活动资源，手动播放、后台恢复与减少动态行为不退化。
- [ ] prompt 正文与首屏元数据分离，请求去重、32 项缓存、4 路批量读取与失败重试有效。
- [ ] 拆分有实测收益的交互代码与 SDK 下载，保持 Cookie 和统计既有语义。
- [ ] 核查版本化资源缓存及部署支持；保留语言、`/_prompts/` 资源和入口别名。
...
- [ ] 完成后分别给出构建审计、浏览器性能、功能回归、部署 HTTP 状态；无真实部署证据时明确标为未验证，不将实验室结果当作线上指标。
