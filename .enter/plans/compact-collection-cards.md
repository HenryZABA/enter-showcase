...
- [ ] 目录页大标题前后、小卡媒体角标及模型卡标题下的 Explore model 全部消失，标题与链接仍可用。
- [ ] 三张目录卡说明改为简单、对应模型且不误导当前空数据的文案；11 语言文本一致。
- [ ] 三种模型单页的眉题、营销副标题、三个分区编号和重复图片索引消失；原有主标题/输入框/返回链接及 FAQ 仍在。
- [ ] 仅 Hot Cases 卡片说明字号减小，默认显示及图片卡原来的悬停/点击行为保持。
- [ ] Case 卡既支持现有静态图片又支持带 poster 的视频，详情海报及本地案例归属保持不变。
- [ ] Trending Prompt 卡可选图片或带 poster 的视频；正文、来源、复制动作与无数据空态不变。
- [ ] 两种视频卡均只在邻近视口加载、可见且非后台时播放，离屏/卸载/减少动态效果能安全停止。

## Verification checklist

- [ ] `pnpm run typecheck`、`pnpm run build`、相关 JSDOM 测试和 `git diff --check` 通过；i18n 校验及最终扫描通过，不删除旧键。
- [ ] 对有数据 Astra 和空数据 Sol/Luna、Opus 核对目录卡及单页：无用户指明的冗余文字，卡片链接和现有内容未丢失。
- [ ] 使用**仅存在于测试**的图片与视频夹具验证 Case/Prompt 两种媒体分支、poster、无媒体、进入/离开视口、后台/前台、减少动态效果和卸载；不向正式库注入假案例。
- [ ] 复用 marketing 严格构建审计，保留现有 JS/字体预算告警；浏览器性能、部署 HTTP 缺证据时明确写“未验证”，不宣称已经提速。
