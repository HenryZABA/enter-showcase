...
export const CaseDetailDialog = ({ entry }: { entry: CaseEntry }) => {
  ...
  return <div className="space-y-6">
    <div className="prompt-detail-actions border-t border-border pt-5">
      ...
      <DownloadPromptButton title={pickCaseTitle(entry, language)} promptUrl={entry.promptUrl} promptKind={entry.promptKind} />
    </div>
    {!entry.remixUrl && <p className="text-xs text-muted-foreground">{t("card.remixUnavailable")}</p>}
